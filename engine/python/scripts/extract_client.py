...
def parse_args() -> argparse.Namespace:  # 解析参数
    parser = argparse.ArgumentParser(description="Extract gRPC client")  # 创建解析器
...
# 分隔  # 中文注释
def build_request(args: argparse.Namespace) -> engine_pb2.ExtractRequest:  # 构建请求
    ...
    return engine_pb2.ExtractRequest(image=image, trace=trace)  # 返回请求
# 分隔  # 中文注释
def main() -> None:  # 主函数
    ...
    response = stub.Extract(request)  # 调用 Extract
    payload = response.payload.payload  # 读取载荷
    payload_fmt = response.payload.payload_format  # 读取格式
    print(f"success={response.success} confidence={response.confidence}")  # 输出状态
    print(f"payload_format={payload_fmt} payload_bytes={payload}")  # 输出载荷
# 分隔  # 中文注释
