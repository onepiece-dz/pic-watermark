...
except ModuleNotFoundError as exc:  # 处理未生成的情况
    raise RuntimeError("Proto stubs not found. Run: python scripts/gen_protos.py") from exc  # 提示生成
# 分隔  # 中文注释
def parse_args() -> argparse.Namespace:  # 解析参数
    parser = argparse.ArgumentParser(description="Embed gRPC client")  # 创建解析器
...
# 分隔  # 中文注释
def build_request(args: argparse.Namespace) -> engine_pb2.EmbedRequest:  # 构建请求
    ...
    payload = common_pb2.WatermarkPayload(  # 组装载荷
        payload=b"demo",  # 示例载荷
        payload_format="raw",  # 载荷格式
    )  # 载荷结束
    options = common_pb2.EmbedOptions(  # 组装策略参数
        strategy_id="mvp",  # 策略ID
        enable_visible=False,  # 是否可见水印
    )  # 策略参数结束
    return engine_pb2.EmbedRequest(  # 返回 Embed 请求
        image=image,  # 输入图片
        payload=payload,  # 水印载荷
        options=options,  # 嵌入策略
        trace=trace,  # 追踪信息
    )  # 请求构建结束
# 分隔  # 中文注释
...
def main() -> None:  # 主函数
    ...
    response = stub.Embed(request)  # 调用 Embed
    output_path = resolve_output(args)  # 解析输出
    Path(output_path).write_bytes(response.image.data)  # 写入结果图片
    print(f"saved: {output_path}")  # 输出结果
# 分隔  # 中文注释
